...
var doEmbed = function() {
	...
	var dataHash = data;
    var str = new TextDecoder().decode(Uint8Array.from(dataHash));
    var enkripsi = enc(str, localStorage.password);    
    ...
    function enc(str, key) {
        ...
        for (let i = 0; i < str.length; i++) {
          ...
          let k = key[i];
          let spasi = 32;
            if (c.charCodeAt(0) == spasi) {
                result += String.fromCharCode(spasi);            
            } else {
                c = (c.charCodeAt(0) + k.charCodeAt(0) - 2 * 33) % 94 + 33;            
        ...
        for (let i = 0; i < str.length; i++) {
          ...
          let spasi = 32;
            if (c.charCodeAt(0) == spasi) {
                ...
                c = (c.charCodeAt(0) - k.charCodeAt(0) + 94) % 94 + 33;            
              result += String.fromCharCode(c);
            }
        }
        return result;
	...
	document.getElementById('outputdiv').textContent = 'Panjang data di embed: ' + data.length + ' bytes\n' +
		'Hasil Enkripsi: ' + enkripsi + '\n' +
		'Estimasi kapasitas citra: ' +s.stats.capacity[1] + ' bytes\n'+
		'Total bytes di embed: ' + s.embedded + ' (' +
		(100*s.embedded/s.stats.capacity[1]).toFixed(2) +'% dari total kapasitas maksimal)\n' +
		'Koefisien berubah: ' + s.changed + ' (' + s.thrown + ')\n';    
};
...
var doExtract = function() {
    ...
    var str = new TextDecoder().decode(Uint8Array.from(dataHash));
    var enkripsi = enc(str, localStorage.password);
    var dekripsi = dec(enkripsi, localStorage.password);
	...
	document.getElementById('outputdiv').textContent = 'Panjang data di ekstraksi: ' + data.length + ' bytes\n' +
        'Hasil ekstraksi: ' + enkripsi + '\n' +
        'Hasil dekripsi: ' + dekripsi;
...
var coverHeight = null;
    ...
    function getPSNR(){
        ...
        var stegoLength = byteStego.length;
        console.log(coverLength);
        var i = 0;
        //sumByte = Math.pow((parseInt(byteCover[0])-parseInt(byteStego[0])),2);
        ...
        document.getElementById('outputdiv').textContent = 'MSE: ' + MSE + '\n' +
        'PSNR: ' + PSNR;
    }
